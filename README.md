...
8. ✅ Admin interface — barber verification queue, coverage areas (add/toggle/delete, map pin), analytics (bookings/revenue/ratings/top barbers + live-editable platform commission % and policy text), and disputes (open/resolved queue with investigate/dismiss/resolve). Customers and barbers can each file a report from a completed/cancelled booking, feeding the disputes queue.
